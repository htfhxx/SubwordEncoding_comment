...
class Data:
    def __init__(self): 
        ...
        self.word_alphabet = Alphabet('word') #使得word_alphabet.instance2index['</unk>']=1    
													#word_alphabet.instances=['</unk>']
        self.biword_alphabet = Alphabet('biword') #同上
    ...
    def build_alphabet(self, input_file):
        ...
        if startB:
            if startS:
                ...
                self.tagScheme = "BIO"   #B 代表当前词是一个组块的开始、I属于一个组块、0不属于任何一个组块 

	#gaz_file是Subword(BPE) embeddings的文件地址
        ...
        for line in in_lines:
            if len(line) > 3:
                ...
                for idx in range(w_length):
                    matched_entity = self.gaz.enumerateMatchList(word_list[idx:])   #返回word_list在gaz_file中匹配到的词list
    ...
    def generate_instance(self, input_file, name):
        self.fix_alphabet() #使data.word_alphabet.keep_growing = False
