...
def train(data, save_model_dir, seg=True):
    ...
    save_data_name = save_model_dir + ".dset" #    save_model_dir = "data/model_zx/saved_model.lstmcrf"
    ...
    for idx in range(data.HP_iteration):   #HP_iteration=50
        ...
        file_test=open('data/model_zx_double_dict/test.txt','a')
        if seg:
...
#修改训练集测试集验证集，修改模型保存地址，修改gpu
    ...
    savemodel ="data/model_zx_double_dict/saved_model.lstmcrf"
    ...
    model_dir = "data/model_zx_double_dict/saved_model.lstmcrf"
    dset_dir = "data/model_zx_double_dict/saved_model.lstmcrf.dset"
        ...
        if data.HP_use_dict==True:
            with codecs.open(dict_path, 'r', 'utf-8') as f:
                for line in f:
                    ...
                    data.dict_wordlists[line] = 1

            with codecs.open(dict_domain_path, 'r', 'utf-8') as f:
                for line in f:
                    line = line.strip().split()[0]
                    data.domain_wordlists[line] = 1
